...
El sistema SHALL exponer `GET /api/v1/courses` que retorna todos los cursos asignados al docente autenticado. Los datos DEBEN provenir de la tabla `courses` filtrada por `teacher_id`.
...
El sistema SHALL exponer `GET /api/v1/courses/{course_id}/students` con soporte de filtrado por estado, búsqueda por nombre y paginación. Los datos DEBEN provenir de la tabla `students`.

#### Scenario: Listado completo sin filtros
- **WHEN** se hace `GET /api/v1/courses/1/students`
- **THEN** la respuesta contiene `students` (array), `total` (int), `page` (int) y `limit` (int)

#### Scenario: Filtro por estado pendiente
- **WHEN** se hace `GET /api/v1/courses/1/students?filter=pendientes`
- **THEN** solo se retornan alumnos con `status = "pendiente"` desde la base de datos

#### Scenario: Búsqueda por nombre
- **WHEN** se hace `GET /api/v1/courses/1/students?search=María`
- **THEN** solo se retornan alumnos cuyo nombre contiene "María" (case-insensitive) usando ILIKE en Postgres

#### Scenario: Paginación
- **WHEN** se hace `GET /api/v1/courses/1/students?page=2&limit=6`
...
El sistema SHALL exponer `GET /api/v1/students/{student_id}` que retorna el perfil completo del alumno incluyendo diagnóstico IA e historial de actividades. Los datos DEBEN provenir de joins reales entre `students`, `ai_diagnoses` y `activities`.

#### Scenario: Alumno existente
- **WHEN** se hace `GET /api/v1/students/1`
...
- **THEN** contiene `text` (string) y `tags` (array de strings cargados desde la columna JSON)
...
- **WHEN** se hace `GET /api/v1/students/99999`
